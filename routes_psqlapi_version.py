...
def daily_report(): 
    ...
    if request.method == "POST" or variableinfo["debugging"] == "true": 
        ...
        if "btn" in request.form or request.method == "GET":
            ...
            elif request.form["btn"] == "API":
                variableinfo["postgresapi"] = "true";
                variableinfo["variable_plot"] = "thd_current";
            elif request.form["btn"] == "Normal":
                variableinfo["postgresapi"] = "false";
                variableinfo["variable_plot"] = "Power";
            
            #setting up buttons to create shashank's api plots
            elif request.form["btn"] == "THD Voltage":
                variableinfo["variable_plot"] = "thd_voltage";
            elif request.form["btn"] == "THD Current":
                variableinfo["variable_plot"] = "thd_current";
            elif request.form["btn"] == "Frequency":
                variableinfo["variable_plot"] = "frequency";

            #setting up normal buttons
        ...
        #setting up buttons to toggle shashank's API
        if variableinfo["postgresapi"] == "false":
            psqlyes = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='API'>");
            psqlno = Markup("<input class='btn btn-secondary' type='submit' name = 'btn' role = 'button' value='Normal' style='background-color:#000000;' disabled>");
        else:
            psqlno = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='Normal'>");
            psqlyes = Markup("<input class='btn btn-secondary' type='submit' name = 'btn' role = 'button' value='API' style='background-color:#000000;' disabled>");
            
            #reuse the power, voltage, thd buttons for these new buttons
            if variableinfo["variable_plot"] == "thd_voltage":
                #power = thd current button
                power_btn = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='THD Current'>");
                #voltage = frequency button
                voltage_btn = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='Frequency'>");
                #thd = thd voltage button (same as before)
                thd_btn = Markup("<input class='btn btn-secondary' type='submit' name = 'btn' role = 'button' value='THD Voltage' disabled style='background-color:#000000;'>");
            elif variableinfo["variable_plot"] == "thd_current":
                thd_btn = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='THD Voltage'>");
                voltage_btn = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='Frequency'>");
                power_btn = Markup("<input class='btn btn-secondary' type='submit' name = 'btn' role = 'button' value='THD Current' disabled style='background-color:#000000;'>");
            elif variableinfo["variable_plot"] == "frequency":
                power_btn = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='THD Current'>");
                thd_btn = Markup("<input class='btn btn-outline-dark' type='submit' name = 'btn' role = 'button' value='THD Voltage'>");
                voltage_btn = Markup("<input class='btn btn-secondary' type='submit' name = 'btn' role = 'button' value='Frequency' disabled style='background-color:#000000;'>");
        
            phasepow_btn = totalpow_btn = size1_btn = size2_btn = size3_btn = size4_btn = interval1_btn = interval2_btn = interval3_btn = interval4_btn = max_btn = min_btn = avg_btn = ""
            
        
        
        if reset != "true" and variableinfo["postgresapi"] == "false":
            #Setting up the buttons specifying how many days of data the user wants to view
    ...
    if request.method == "GET" or set_btn == 1:    
        ...
        if variableinfo["postgresapi"] == "true":
            if variableinfo["variable_plot"] == 'thd_voltage':
                description = "Currently Viewing THD Voltage";
            elif variableinfo["variable_plot"] == 'thd_current':
                description = "Currently Viewing THD Current";
            else:
                description = "Currently Viewing Frequency";
        elif variableinfo["total"] == "false" and not split_phase and variableinfo["variable_plot"] == "Power":
...
def GetReportData(day): 
    with psycopg2.connect(CONNECTION) as conn:        
        ...
        #Doing a query from midnight until the present time using variables
        today = date.today()
        midnight = datetime.combine(today, datetime.min.time()).isoformat()
        midnight = midnight + 'Z'
        current = datetime.now(timezone.utc).isoformat()
        
        if variableinfo["postgresapi"] == "true":
            phaseadata = apiclass.db_retrieve('phasea', variableinfo["variable_plot"], midnight, current, CONNECTION) # <-- querying data
            datalength = len(phaseadata)
            for row in phaseadata:    
                if datalength > 1:
                    phasesh['time'].append(row['time']);
                    phasesh[variableinfo['variable_plot']].append(row[variableinfo['variable_plot']]);
                
                datalength -= 1;
        
        #this works as well
        #phaseadata = apiclass.db_retrieve('phasea', 'frequency', '2022-08-17T00:00:00.000Z', '2022-08-17T22:15:59.879Z', CONNECTION)
          
        #shows that the other two functions in the api work
        """logger.info("Available tables: %s tables", len(array_tables))
        for i in range(0, len(array_tables)):
            logger.info(array_tables[i]);
            
        logger.info("Fields in phasea table: %s fields", len(array_fields))
        for i in range(0, len(array_fields)):
            logger.info(array_fields[i]);"""
        ...
        #Testing shashanks api by plotting
        if variableinfo["postgresapi"] == "true":
            (bk_script, bk_div) = components(
                    #phasesh is an object storing data pulled using shashank's API
                    graphs.plot_shashank(
                        phasesh['time'], variableinfo["variable_plot"], phasesh[variableinfo["variable_plot"]]
                    )
                )
        elif variableinfo["total"] == "true":
